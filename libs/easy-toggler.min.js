(function () {
  ...
  function EasyTogglerHandler(event) {
    ...
    if (!EY_BTN) {
      ...
      Array.from(ey_rcoe_block_targets).forEach.call(
        ey_rcoe_block_targets,
        function (ey_rcoe_block_target) {
          let ey_rcoe_block =
              ey_rcoe_block_target.getAttribute("data-easy-toggle"),
            ey_rcoe_block_class =
              ey_rcoe_block_target.getAttribute("data-easy-class");
          if (!event.target.closest(ey_rcoe_block)) {
            try {
              document
                .querySelector(ey_rcoe_block)
                .classList.remove(ey_rcoe_block_class);
            } catch (ey_error) {
              console.warn(
                "EasyToggler.js : Блок " + ey_rcoe_block + " не существует"
              );
            }
          }
        }
